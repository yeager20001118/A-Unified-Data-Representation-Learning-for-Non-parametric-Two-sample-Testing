...
#     pickle.dump(summary, f)
    ...
    h_s, _, _ = TST_C2ST_D(S, n_train, N_PER, alpha,
                           discriminator, device, dtype)
    h_l, _, _ = TST_LCE_D(S, n_train, N_PER, alpha,
                          discriminator, device, dtype)
    print(h_s, h_l)
    ...
    for k in range(N_TEST):
        H_C2ST_S[k], _, _ = TST_C2ST_D(
            S_test, n_train, N_PER, alpha, discriminator, device, dtype)
        H_C2ST_L[k], _, _ = TST_LCE_D(
            S_test, n_train, N_PER, alpha, discriminator, device, dtype)
